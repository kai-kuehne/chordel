...
// Improvements by Kai Kühne:
// - realloc with size 0 is implementation-defined; check for newsize == 0.
...
extern inline bool bitset_for_each(const bitset_t* b, bitset_iterator iterator, void* ptr);
extern inline size_t bitset_next_set_bits(const bitset_t* bitset, size_t* buffer, size_t capacity, size_t* startfrom);
