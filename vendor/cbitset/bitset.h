...
/* For advanced users: Grow the bitset so that it can support newarraysize * 64 bits with padding. Return true
...
/* shifts all bits by 's' positions so that the bitset representing values 1,2,10 would represent values 1+s,
 * 2+s, 10+s */
void bitset_shift_left(bitset_t* bitset, size_t s);

/* shifts all bits by 's' positions so that the bitset representing values 1,2,10 would represent values 1-s,
 * 2-s, 10-s, negative values are deleted */
...
/* compute the union in-place (to b1), returns true if successful, to generate a new bitset first call
 * bitset_copy */
...
/* compute the symmetric difference in-place (to b1), return true if successful, to generate a new bitset
 * first call bitset_copy */
...
size_t
bitset_symmetric_difference_count(const bitset_t* CBITSET_RESTRICT b1, const bitset_t* CBITSET_RESTRICT b2);
...
bitset_next_set_bits(const bitset_t* bitset, size_t* buffer, size_t capacity, size_t* startfrom) {
    if (capacity == 0)
